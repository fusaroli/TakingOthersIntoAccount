...
Pars <- xxx %>% group_by(Attention) %>% dplyr::summarize(wSelf=mean(wSelf),wOthers=mean(wOthers),aSelf=mean(aSelf),aOthers=mean(aOthers),bias=mean(bias),bConfidence=mean(bConfidence))
Pars$wSelf_scale <- inv_logit_scaled(Pars$wSelf, 0.5, 1)
Pars$wOthers_scale <- inv_logit_scaled(Pars$wOthers + Pars$bConfidence, 0.5, 1)
Pars$wOthersLowConf_scale <- inv_logit_scaled(Pars$wOthers + (Pars$bConfidence*0.5), 0.5, 1)
Pars$aSelf_scale <- exp(Pars$aSelf)
...
hypothesis(circular_AnhedoniaHallucinations_m,"aOthers_AnhedoniaAnhedoniaAnhedoniaS < 0")
```
...
SymptomsCor_f <- bf(
  mvbind(Hallucinations,Delusions,Bizarre,FormalThought,Flattening,Avolition,Anhedonia,Attention) ~ 1 + (1|p|Subject)
) 
...
unique(Demo[,"Hallucinations"])
unique(Demo[,"Delusions"])
unique(Demo[,"Bizarre"])
unique(Demo[,"FormalThought"])
unique(Demo[,"Flattening"])
unique(Demo[,"Avolition"])
unique(Demo[,"Anhedonia"])
unique(Demo[,"Attention"])

DemoD <- Demo %>% subset(Subject < 200)
DemoD[,c("Hallucinations","Delusions","Bizarre","FormalThought","Flattening","Avolition","Anhedonia","Attention")] <- DemoD[,c("Hallucinations","Delusions","Bizarre","FormalThought","Flattening","Avolition","Anhedonia","Attention")] + 1
...
SymptomsCor_m <- brm(
  ...
  file = "models/SymptomsCor2"
)

summary(SymptomsCor_m)
SymptomsCor_m <- add_criterion(SymptomsCor_m, criterion = "loo", file="models/SymptomsCor2")
```
